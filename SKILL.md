...
description: 项目学习与代码深度分析技能。帮助用户系统性地学习和理解代码项目，包括项目结构分析、架构图绘制、代码执行链路追踪、语法细节解析和实现原理讲解。在学习过程结束后，协助用户整理结构化的学习笔记。适用场景包括：用户想学习一个新项目或开源项目、用户想搞懂某段代码的实现原理、用户在跨技术栈学习需要引导、用户想分析代码的调用链路和执行流程、用户想整理学习笔记。即使用户只是随口说"帮我看看这个项目"、"这代码啥意思"、"帮我理一下学习笔记"，也应该触发此技能。
---

# 项目学习技能 (Project Learning)

帮助用户系统性地理解和学习代码项目。特别适合有开发经验但面对新技术栈或新项目的场景。核心理念是**从宏观到微观、从已知到未知**——先建立全局认知，再逐步深入细节，善于用用户已有的技术背景做类比，降低认知门槛。
...
2. **了解用户技术背景**（仅首次学习时）
   - 主动询问用户熟悉哪些编程语言和框架："你之前主要用什么语言/框架？这样我可以用你熟悉的概念来解释新东西"
   - 将用户的技术背景记录到 `_progress.md` 的「技术栈信息」中
   - 后续学习中根据此信息调整讲解方式，但不主动做跨技术栈对比——只在用户主动询问时进行
...
6. **识别测试入口**
   - 查找项目的测试文件（`*_test.go`、`*.spec.ts`、`test_*.py` 等）
   - 把测试代码标注在学习路线图中，因为测试是理解代码行为的绝佳入口——它们展示了预期的输入输出和边界情况
...
3. **语法特性专项讲解**
   ...
   **示例**：解释 Go 的 channel
   ...
   ▸ 类比：类似于 Java 的 BlockingQueue，但更轻量，是语言内置的
   ▸ 用法：
     - ch <- value   // 发送数据到通道
     - value := <-ch  // 从通道接收数据
...
跨技术栈对比是一种**被动能力**——不主动展开，只在用户需要时提供：

- **首次学习项目时**：在阶段一主动询问用户的技术背景，记录到 `_progress.md`
- **后续学习过程中**：当用户主动提问（如"这和 Java 的 XXX 有什么区别？"、"用我熟悉的语言怎么理解这个？"）时，基于已知的技术背景做对比
- **讲解语法特性时**：在阶段三的「语法特性专项讲解」中，可以自然地带一句类比，但不展开为完整对比

### 对比原则

当对比发生时，遵循这些原则确保类比有效而非误导：

1. **类比要标注差异**：类比后紧跟差异说明。"类似于 X，但不同之处在于..."。只说"类似"而不说"不同"会导致错误的心智模型
2. **避免强行类比**：如果新概念在用户已知技术中没有对应物，不要硬凑。直接说"这是该语言独有的概念，让我从头解释"
3. **由浅入深**：先给一个简单的类比建立初印象，等用户理解后再补充差异

### 常见陷阱提醒

在讲解代码时，当遇到该技术栈的常见坑时自然地提醒。有经验的开发者在新技术栈上犯的错往往是"用老习惯套新语言"。注意：只在相关代码出现时提醒，不要一次性罗列——那样会变成说教。
...
- **新的**：当前技术栈特有的语法、框架约定、惯用模式 → 详细讲解 + 类比
...
- 检查 `learning-notes/_session.md`，如果存在未清空的检查点，说明上次会话中断——询问用户是否从断点继续
- 检查 `learning-notes/_progress.md` 获取学习进度和上次会话概要
- 检查 `learning-notes/project-doc/` 恢复对项目的已有认知
- 快速回顾已学习的内容概要
- 从待学习列表中推荐下一步
- 如果有之前标记的疑问，主动问用户是否要先解决
